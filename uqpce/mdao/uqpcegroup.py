...
if __name__ == '__main__':
    ...
    prob.model.add_subsystem(
        ...
        promotes_inputs=['*'], promotes_outputs=[('f_abxy', 'resp')]
    )

    prob.model.add_subsystem(
        'add_units',
        om.ExecComp('f_abxy = 2*resp', f_abxy={'units': 'N'}, shape=(6,)),
        promotes_inputs=['resp'], promotes_outputs=['f_abxy']
    )
    ...
    print('Interpolated CI: ', np.min(np.quantile(np.reshape(prob.get_val(f'f_abxy:resampled_responses'), (-1, aleat_cnt)), sig/2, axis=1)))
