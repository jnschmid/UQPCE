"""
GMM-UQPCE Optimization Example
...
class ImprovedFunction(om.ExplicitComponent):
   ...
   a1: GMM variable (multimodal - creates different optimal regions)
   a2: Uniform variable (affects y optimum)
   a3: Lognormal variable (small contribution)
   """
   ...
   def compute(self, inputs, outputs):
       ...
       # Function with coupling term -3*x*a1
   ...
   def compute_partials(self, inputs, partials):
       ...
       # df/dx = 2(x - a1) - 3*a1 + 0.04*a3*x
       partials['f', 'x'] = (2*(x - a1) - 3*a1 + 0.04*a3*x).reshape(-1, 1)
       
       # df/dy = 0.2*(y - a2) + 0.04*a3*y
       partials['f', 'y'] = (0.2*(y - a2) + 0.04*a3*y).reshape(-1, 1)
       
       # df/da1 = -2*(x - a1) - 3*x = -2*x + 2*a1 - 3*x = -5*x + 2*a1
...
def monte_carlo_eval(x, y, n_samples=500000):
   """Evaluate function using Monte Carlo"""
...
def main():
   print("\n" + "="*70)
   print("GMM-UQPCE OPTIMIZATION EXAMPLE")
   print("="*70)
   ...
   print(f"Mean = {f_mean:.4f}, Std = {np.sqrt(f_var):.4f}")
   ...
   f_initial_mc = monte_carlo_eval(2.0, -2.0)
   f_final_mc = monte_carlo_eval(x_uq, y_uq)
   f_det_mc = monte_carlo_eval(x_det, y_det)
   
   print(f"Initial (2, -2): Mean = {np.mean(f_initial_mc):.4f}, Std = {np.std(f_initial_mc):.4f}")
   print(f"Robust ({x_uq:.3f}, {y_uq:.3f}): Mean = {np.mean(f_final_mc):.4f}, Std = {np.std(f_final_mc):.4f}")
   print(f"Deterministic ({x_det:.3f}, {y_det:.3f}): Mean = {np.mean(f_det_mc):.4f}, Std = {np.std(f_det_mc):.4f}")
   ...
   # Plot individual weighted components
   ...
   # Use mean values for contour plot
   ...
   ax = axes[0, 2]
   all_data = np.concatenate([f_initial_pce.flatten(), f_initial_mc])
   ...
   ax.hist(f_initial_mc, bins=bins, alpha=0.5, density=True, 
   ...
   ax = axes[1, 0]
   all_data = np.concatenate([f_final_pce.flatten(), f_final_mc])
   bins = np.linspace(np.percentile(all_data, 1), np.percentile(all_data, 99), 35)
   
   ax.hist(f_final_pce.flatten(), bins=bins, alpha=0.5, density=True, 
           color='blue', label='PCE')
   ax.hist(f_final_mc, bins=bins, alpha=0.5, density=True, 
           color='red', label='Monte Carlo')
   ax.set_xlabel('Objective Value')
   ax.set_ylabel('Density')
   ...
   table_data = [
       ...
       ['Mean (MC)', f'{np.mean(f_det_mc):.3f}', f'{np.mean(f_final_mc):.3f}'],
       ['Std (MC)', f'{np.std(f_det_mc):.3f}', f'{np.std(f_final_mc):.3f}']
   ]
   ...
   # Color code the shift rows
   for i in [3, 4]:
       table[(i, 2)].set_facecolor('#ffe6e6' if abs(float(table_data[i][2])) > 0.5 else '#e6ffe6')
   
   ax.set_title('Optimization Results', fontsize=12, fontweight='bold')
   
   # 6. Comparison of distributions at deterministic vs robust points
   ax = axes[1, 2]
   
   bins = np.linspace(min(f_det_mc.min(), f_final_mc.min()), 
                     max(f_det_mc.max(), f_final_mc.max()), 35)
   
   ax.hist(f_det_mc, bins=bins, alpha=0.5, density=True, 
           color='red', label=f'Det. point ({x_det:.2f}, {y_det:.2f})')
   ax.hist(f_final_mc, bins=bins, alpha=0.5, density=True, 
           color='blue', label=f'Robust point ({x_uq:.2f}, {y_uq:.2f})')
   
   ax.axvline(np.mean(f_det_mc), color='red', linestyle='--', alpha=0.7, label=f'Det. mean: {np.mean(f_det_mc):.2f}')
   ax.axvline(np.mean(f_final_mc), color='blue', linestyle='--', alpha=0.7, label=f'Rob. mean: {np.mean(f_final_mc):.2f}')
   
   ax.set_xlabel('Objective Value')
   ax.set_ylabel('Density')
   ax.set_title('Det. vs Robust Point Distributions (MC)')
   ...
   print(f"\nThe coupling term -3*x*a1 creates different optimal regions:")
   print(f"  - Deterministic (using a1 mean={variables[0].get_mean():.3f}): ({x_det:.3f}, {y_det:.3f})")
   print(f"  - Robust (considering GMM modes at -1, 0, 2): ({x_uq:.3f}, {y_uq:.3f})")
   print("="*70 + "\n")
